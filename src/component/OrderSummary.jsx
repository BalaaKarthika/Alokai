...
export default function OrderSummary() {
  ...
  useEffect(() => {
    clearTimeout(exceedTimer.current);
    exceedTimer.current = window.setTimeout(() => setExceedAlert(false), 5000);
    return () => clearTimeout(exceedTimer.current);
  }, [exceedAlert]);
  ...
  return (
    <div>
      ...
      {positiveAlert && (
        <div className="fixed top-16 left-1/2 transform -translate-x-1/2 flex items-center bg-green-300 text-white text-lg font-semibold py-2 px-3 rounded-md shadow-lg z-50">
          <span>Promo code applied successfully!</span>
        </div>
      )}
      {informationAlert && (
        <div className="fixed top-16 left-1/2 transform -translate-x-1/2 flex items-center bg-blue-300 text-white text-lg font-semibold py-2 px-3 rounded-md shadow-lg z-50">
